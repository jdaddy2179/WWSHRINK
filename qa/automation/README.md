...
| `WWShrink_KCLConfig_FullTableAnalysis.sql` | Same analysis for the **Config** DB: `Windward_Config` vs `Windward_Config_KCL`. Lines up with the latest `TrimShrink_Run` (Config shrink), so the step-log view matches the config trim steps. Results captured in `WWShrink_KCLConfig_TableAnalysis_Results.md` | TC-P4-04 / TC-P4-05 |
