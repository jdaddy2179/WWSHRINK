-- WW DataShrink — KCL FULL TABLE-LEVEL ANALYSIS (source vs target)
-- =================================================================
...
-- -----------------------------------------------------------------
-- 1) SOURCE table inventory — windward_commercial
-- -----------------------------------------------------------------
drop table if exists #src_tbl;

;with src_rows as (
    select object_id, sum(rows) as row_count
    from [windward_commercial].sys.partitions
    where index_id in (0,1)              -- heap (0) or clustered index (1)
    group by object_id
),
src_size as (
    select p.object_id, sum(a.total_pages) as total_pages
    from [windward_commercial].sys.partitions p
    join [windward_commercial].sys.allocation_units a on a.container_id = p.partition_id
...
into #src_tbl
from [windward_commercial].sys.tables t
join [windward_commercial].sys.schemas sch on sch.schema_id = t.schema_id
left join src_rows r  on r.object_id  = t.object_id
left join src_size sz on sz.object_id = t.object_id
where t.is_ms_shipped = 0;

-- -----------------------------------------------------------------
-- 2) TARGET table inventory — windward_KCL
-- -----------------------------------------------------------------
drop table if exists #tgt_tbl;

;with tgt_rows as (
    select object_id, sum(rows) as row_count
    from [windward_KCL].sys.partitions
    where index_id in (0,1)
    group by object_id
),
tgt_size as (
    select p.object_id, sum(a.total_pages) as total_pages
    from [windward_KCL].sys.partitions p
    join [windward_KCL].sys.allocation_units a on a.container_id = p.partition_id
    group by p.object_id
)
select sch.name as schema_name,
       t.name   as table_name,
       ...
       cast(isnull(sz.total_pages, 0) * 8 / 1024.0 as decimal(18,2)) as reserved_mb
into #tgt_tbl
from [windward_KCL].sys.tables t
join [windward_KCL].sys.schemas sch on sch.schema_id = t.schema_id
...
select
    ...
    commercial_mb   = isnull(s.reserved_mb, 0),
    kcl_mb          = isnull(t.reserved_mb, 0),
